...
const updateUser = async (req, res) => {
  ...
  if (isValid(userData.email) && isValidEmail(userData.email)) {           
    try {
      ...
      if (isEmailExist) throw new Error(`This email ${userData.email} id is already registerd`) 
    } catch (error) {
  ...
  try {
    ...
    const updatedUser = await UserModel.findByIdAndUpdate(authId, { $set: userData }, { new: true });  
